...
public class SBertRM3ExpansionTest<T> extends LuceneTestCase {
	...
	public void testEmptyRelevanceFeedbackReturnsOriginalQuery() {
		...
		Map<Integer, List<String>> queryIdToDocuments = Map.of(2, Arrays.asList());
		SBertQueryExpansionConfiguration config = new SBertQueryExpansionConfiguration("10", "10", "0.5", "0.3", queryIdToDocuments, tempDir2.toString());
	...
	public void testSorting() {
		List<Pair<Terms, Double>> tmp = Arrays.asList(Pair.of(null, 0.1d), Pair.of(null, 0.2d));
		String expected = "[(null,0.2), (null,0.1)]";
		String actual = SBertRm3Reranker.sortedRelevanceFeedback(tmp, 10).toString();
	...
	public void testSorting2() {
		List<Pair<Terms, Double>> tmp = Arrays.asList(Pair.of(null, 0.2d), Pair.of(null, 0.1d));
		String expected = "[(null,0.2), (null,0.1)]";
		String actual = SBertRm3Reranker.sortedRelevanceFeedback(tmp, 10).toString();
	...
	public void testSorting3() {
		List<Pair<Terms, Double>> tmp = Arrays.asList(Pair.of(null, 0.2d), Pair.of(null, 0.4d), Pair.of(null, 0.2d), Pair.of(null, 0.3d));
		String expected = "[(null,0.4), (null,0.3)]";
		String actual = SBertRm3Reranker.sortedRelevanceFeedback(tmp, 2).toString();
	...
	public void testRelevanceFeedbackWithSingleExpansionTermOriginalQuery2() {
		String expected = "(contents:some)^0.15252784 (contents:citi)^0.07626392 (contents:test)^0.27120823 (contents:here)^0.5";
	...
	public void testRelevanceFeedbackWithoutMatchReturnsOriginalQuery() {
		...
		SBertQueryExpansionConfiguration config = new SBertQueryExpansionConfiguration("10", "1", "0.5", "10", queryIdToDocuments, tempDir2.toString());
		String actual = actualRM3Expansion(3, "text", config);
		
		Assert.assertEquals(expected, actual);
	}
	
	private String actualRM3Expansion(int queryId, String originalQuery, SBertQueryExpansionConfiguration config) {
		SBertRm3Reranker<T> reranker = new SBertRm3Reranker<>(new EnglishAnalyzer(), config);
